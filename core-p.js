...
function fcas_renderizarHTMLMenu(config,isPt,nav,sidebar){let htmlMenu='';let totalGeral=0;config.categorias.forEach(function(g){if(g&&Array.isArray(g.botoes))totalGeral+=g.botoes.length});let htmlSidebar='<button class="fcas-sidebar-item active" data-target="all">'+(isPt?'Todos':'All')+'<span class="fcas-badge">'+totalGeral+'</span></button><hr class="fcas-sidebar-divider">';config.categorias.forEach(function(g,i){if(g&&typeof g==='object'&&typeof g.nome==='string'&&Array.isArray(g.botoes)){const gid='fcas-grupo-'+i;htmlMenu+='<div class="cat-group" id="'+gid+'"><h3>'+g.nome+' ('+g.botoes.length+')</h3>';g.botoes.forEach(function(b){if(b&&typeof b==='object'&&typeof b.id==='string'&&typeof b.texto==='string'){htmlMenu+='<button id="'+b.id+'">'+b.texto+'</button>'}});htmlMenu+='</div>';htmlSidebar+='<button class="fcas-sidebar-item" data-target="'+gid+'">'+g.nome+'<span class="fcas-badge">'+g.botoes.length+'</span></button>'}});nav.innerHTML=htmlMenu;sidebar.innerHTML=htmlSidebar;}
