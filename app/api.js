...
function fcasfs_lightbox_converterHexParaRgba(corHex,opacidade,importante){const hex=(corHex||'#000000').trim().replace('#','');let op=opacidade!==undefined?parseFloat(opacidade):1;if(op>1)op=op/100;let r=0,g=0,b=0;if(hex.length===3){r=parseInt(hex[0]+hex[0],16);g=parseInt(hex[1]+hex[1],16);b=parseInt(hex[2]+hex[2],16);}else if(hex.length===6){r=parseInt(hex.slice(0,2),16);g=parseInt(hex.slice(2,4),16);b=parseInt(hex.slice(4,6),16);}const imp=importante===true||importante==='true'?' !important':'';return `rgba(${r},${g},${b},${op})${imp}`;}
