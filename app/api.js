...
function fcafs_lightbox_gerarAvisoConteudoAdulto(config){if(!config||!config.isAdult)return'';const textos={pt:{titulo:"Aviso de Conteúdo Adulto",mensagem:"Este site contém conteúdo para maiores de 18 anos. Você deseja continuar?",sim:"Sim",nao:"Não"},en:{titulo:"Adult Content Warning",mensagem:"This website contains content for people over 18. Do you wish to continue?",sim:"Yes",nao:"No"}};const idioma=textos[config.lang]?config.lang:'pt';const traducao=textos[idioma];const btnSim=config.onYes?`<button onclick="${config.onYes}" class="sim">${traducao.sim}</button>`:'';const btnNao=config.onNo?`<button onclick="${config.onNo}" class="nao">${traducao.nao}</button>`:'';return `<div class="modal-aviso-container"><style>.modal-aviso-container{width:90%;background-color:#111;display:flex;justify-content:center;align-items:center;z-index:99999;font-family:Arial,sans-serif}.modal-aviso-container div{background-color:#fff;padding:30px;border-radius:8px;text-align:center;max-width:400px;box-shadow:0 4px 15px rgba(0,0,0,0.3)}.modal-aviso-container h2{margin:0 0 15px 0;color:#333}.modal-aviso-container p{margin:0 0 25px 0;color:#666;line-height:1.5}.modal-aviso-container div div{background-color:transparent;padding:0;border-radius:0;box-shadow:none;max-width:none;display:flex;justify-content:space-around}.modal-aviso-container button{padding:10px 25px;border:none;border-radius:4px;color:#fff;cursor:pointer;font-weight:bold}.modal-aviso-container .sim{background-color:#28a745}.modal-aviso-container .nao{background-color:#dc3545}  @media (max-width:480px){.modal-aviso-container{width:100%!important;padding:15px;box-sizing:border-box}.modal-aviso-container div{width:100%;padding:20px;box-sizing:border-box}.modal-aviso-container div div{gap:15px}.modal-aviso-container button{flex:1;padding:12px 0}}  </style><div><h2>${traducao.titulo}</h2><p>${traducao.mensagem}</p><div>${btnSim}${btnNao}</div></div></div>`;}
