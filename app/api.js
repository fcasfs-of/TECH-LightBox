...
function fcafs_lightbox_createSlideshow(c){const s=c.slides||[],fx=['fade','zoom','cube'].includes(c.effect)?c.effect:'slide';if(!s.length)return'';const id='ss_'+Math.random().toString(36).substr(2,9);if(!document.getElementById('ss-style')){const e=document.createElement('style');e.id='ss-style';e.textContent='.js-ss-box{background: rgba(17, 17, 17, 0.8);position:relative;width:100%;height:100%;overflow:hidden;border-radius:8px;perspective:1200px}.js-ss-track{display:flex;width:100%;height:100%}.js-ss-track.fx-slide{transition:transform .5s cubic-bezier(.25,.46,.45,.94)}.js-ss-track.fx-fade,.js-ss-track.fx-zoom,.js-ss-track.fx-cube{position:relative;transform-style:preserve-3d}.js-ss-img{min-width:100%;height:100%;box-sizing:border-box;position:relative}.js-ss-track:not(.fx-slide) .js-ss-img{position:absolute;top:0;left:0;opacity:0;z-index:1;pointer-events:none;transition:all .6s cubic-bezier(.4,.0,.2,1)}.js-ss-track:not(.fx-slide) .js-ss-img.is-active{opacity:1;position:relative;z-index:2;pointer-events:auto}.js-ss-track.fx-zoom .js-ss-img{transform:scale(0.85)}.js-ss-track.fx-zoom .js-ss-img.is-active{transform:scale(1)}.js-ss-cube .js-ss-img{transform:rotateY(90deg) translateZ(100px);transform-origin:center center}.js-ss-track.fx-cube .js-ss-img.is-active{transform:rotateY(0deg) translateZ(0)}.js-ss-caption{position:absolute;bottom:16px;left:16px;right:16px;background:rgba(0,0,0,0.75);color:#fff;padding:12px 20px;font-family:system-ui,-apple-system,sans-serif;text-align:center;font-size:14px;border-radius:6px;backdrop-filter:blur(4px);z-index:5}';document.head.appendChild(e)}if(!window.jsSS){window.jsSS={m:(i,d,m,f)=>{window.jsSS[i]=window.jsSS[i]||0;const old=window.jsSS[i];window.jsSS[i]=(window.jsSS[i]+d+m)%m;const t=document.getElementById(i);if(t){if(f==='slide'){t.style.transform=`translateX(-${window.jsSS[i]*100}%)`}else{const ch=t.children;if(ch[old])ch[old].classList.remove('is-active');if(ch[window.jsSS[i]])ch[window.jsSS[i]].classList.add('is-active')}}}}}return`<div class="js-ss-box" style="height:100%;width:100%"><div class="js-ss-track fx-${fx}" id="${id}">${s.map((i,x)=>`<div class="js-ss-img${fx!=='slide'&&x===0?' is-active':''}"><img src="${i.url}" alt="${i.alt||''}" style="pointer-events:none;width:100%;height:100%;object-fit:contain;display:block">${c.showCaptions&&i.caption?`<div class="js-ss-caption">${i.caption}</div>`:''}</div>`).join('')}</div>${c.showControls&&s.length>1?`<button style="cursor:pointer;position:absolute;top:50%;transform:translateY(-50%);width:40px;height:40px;background:rgba(0,0,0,.5);border:none;border-radius:50%;display:flex;align-items:center;justify-content:center;z-index:10;padding:0;left:12px" onclick="jsSS.m('${id}',-1,${s.length},'${fx}')"><svg viewBox="0 0 24 24" style="width:24px;height:24px;fill:#fff"><path d="M15.41 7.41L14 6l-6 6 6 6 1.41-1.41L10.83 12z"/></svg></button><button style="cursor:pointer;position:absolute;top:50%;transform:translateY(-50%);width:40px;height:40px;background:rgba(0,0,0,.5);border:none;border-radius:50%;display:flex;align-items:center;justify-content:center;z-index:10;padding:0;right:12px" onclick="jsSS.m('${id}',1,${s.length},'${fx}')"><svg viewBox="0 0 24 24" style="width:24px;height:24px;fill:#fff"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/></svg></button>`:''}</div>`}

function fcafs_lightbox_gerarAvisoConteudoAdulto(config){if(!config||!config.isAdult)return'';const textos={pt:{titulo:"Aviso de Conteúdo Adulto",mensagem:"Este lightbox contém conteúdo para maiores de 18 anos. Você deseja continuar?",sim:"Sim",nao:"Não"},en:{titulo:"Adult Content Warning",mensagem:"This lightbox contains content for people over 18. Do you wish to continue?",sim:"Yes",nao:"No"}};const idioma=textos[config.lang]?config.lang:'pt';const traducao=textos[idioma];const svgWarning=`<svg xmlns="http://w3.org" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle; margin-right: 8px; color: #dc3545;"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>`;const svgCheck=`<svg xmlns="http://w3.org" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle; margin-right: 6px;"><polyline points="20 6 9 17 4 12"/></svg>`;const svgX=`<svg xmlns="http://w3.org" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle; margin-right: 6px;"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>`;const btnSim=config.onYes?`<button onclick="${config.onYes}" class="sim">${svgCheck}${traducao.sim}</button>`:'';const btnNao=config.onNo?`<button onclick="${config.onNo}" class="nao">${svgX}${traducao.nao}</button>`:'';return `<div class="modal-aviso-container"><style>.modal-aviso-container{width:90%;background-color:#111;display:flex;justify-content:center;align-items:center;z-index:99999;font-family:Arial,sans-serif}.modal-aviso-container div{background-color:#fff;padding:30px;border-radius:8px;text-align:center;max-width:400px;box-shadow:0 4px 15px rgba(0,0,0,0.3)}.modal-aviso-container h2{margin:0 0 15px 0;color:#333;display:flex;align-items:center;justify-content:center}.modal-aviso-container p{margin:0 0 25px 0;color:#666;line-height:1.5}.modal-aviso-container div div{background-color:transparent;padding:0;border-radius:0;box-shadow:none;max-width:none;display:flex;justify-content:space-around}.modal-aviso-container button{padding:10px 25px;border:none;border-radius:4px;color:#fff;cursor:pointer;font-weight:bold;display:flex;align-items:center;justify-content:center}.modal-aviso-container .sim{background-color:#28a745}.modal-aviso-container .nao{background-color:#dc3545}@media (max-width:480px){.modal-aviso-container{width:100%!important;padding:15px;box-sizing:border-box}.modal-aviso-container div{width:100%;padding:20px;box-sizing:border-box}.modal-aviso-container div div{gap:15px}.modal-aviso-container button{flex:1;padding:12px 0}}</style><div><h2>${svgWarning}${traducao.titulo}</h2><p>${traducao.mensagem}</p><div>${btnSim}${btnNao}</div></div></div>`;}

function fcas_lightbox_mapa(dados){if(!dados||!dados.locais||0===dados.locais.length)return"";const{locais=[],tema='claro',habilitar3D=false,formatoMapa='',tamanhoMapa='100%'}=dados,e=tema==="escuro",bg=e?'#1e1e1e':'#fff',brd=e?'#333':'#f0f0f0',mapB=e?'#2a2a2d':'#f2f2f2',mapS=e?'#444':'#d1d1d1',idSvg='fcasSvg'+Math.floor(Math.random()*1e5),dPath='M35,10 C45,8 55,12 65,15 C75,18 85,25 90,35 C95,45 92,55 85,65 C78,75 70,82 60,88 C50,94 38,95 28,90 C18,85 10,75 8,62 C6,49 12,38 18,28 C24,18 28,12 35,10 Z',fundo3D=habilitar3D?'perspective(100vw) rotateX(55deg) rotateY(0deg) rotateZ(-20deg)':'none',sombr3D=habilitar3D?`<path d="${dPath}" fill="${e?'#111113':'#d8d8d8'}" transform="translate(0, 5) scale(1)"/>`:'',estiloFormato='circle'===formatoMapa?'border-radius:50%;aspect-ratio:1/1;':'square'===formatoMapa?'border-radius:12px;aspect-ratio:1/1;':'border-radius:12px;';let c=locais.map((t,s)=>{const o=Number(t.x)||50,a=Number(t.y)||50,pCor=t.cor||'#f472b6',pFmt=t.formato==='square'?'square':'circle',desenho='square'===pFmt?`<rect x="${o-1.5}" y="${a-1.5}" width="3" height="3" fill="${pCor}" style="transition:transform .2s,fill .2s;transform-origin:${o}px ${a}px"/>`:`<circle cx="${o}" cy="${a}" r="1.5" fill="${pCor}" style="transition:transform .2s,fill .2s;transform-origin:${o}px ${a}px"/>`;return`<g class="fcas-map-pin" style="cursor:pointer" onclick="const s=document.getElementById('${idSvg}');const t=this.querySelector('.fcas-pin-txt');s.querySelectorAll('.fcas-pin-txt').forEach(e=>{if(e!==t){e.style.display='none';e.classList.remove('fcas-fixo')}});if(s.getAttribute('viewBox')==='0 0 100 100'){s.setAttribute('viewBox','${o-15} ${a-15} 30 30');t.setAttribute('x','${o}');t.setAttribute('y','${a-12}');t.style.fontSize='1px';t.setAttribute('dy','0');t.style.display='block';t.classList.add('fcas-fixo')}else{s.setAttribute('viewBox','0 0 100 100');t.setAttribute('x','${o}');t.setAttribute('y','${a}');t.style.fontSize='3px';t.setAttribute('dy','-4');t.style.display='none';t.classList.remove('fcas-fixo')}event.stopPropagation();">${desenho}<circle cx="${o}" cy="${a}" r="3.5" fill="none" stroke="${pCor}" stroke-width="0.4" opacity="0.3"/><text x="${o}" y="${a}" dy="-4" text-anchor="middle" font-size="3px" font-family="sans-serif" font-weight="bold" fill="${e?'#fff':'#111'}" class="fcas-pin-txt" style="display:none;pointer-events:none;background:${bg}">${t.nome}</text></g>`}).join('');return`<style>.fcas-map-pin:hover circle:first-child,.fcas-map-pin:hover rect{transform:scale(1.4);fill:#38bdf8!important}.fcas-map-pin:hover .fcas-pin-txt:not(.fcas-fixo){display:block!important}</style><div class="fcas-map-card" style="background:${bg};box-shadow:0 10px 25px ${e?'rgba(0,0,0,0.5)':'rgba(0,0,0,0.1)'};box-sizing:border-box;display:block;margin:0;perspective:100vw;width:${tamanhoMapa};max-width:100%;${estiloFormato}"><div style="background:${e?'#121212':'#fafafa'};border-radius:inherit;border:1px solid ${brd};padding:10px;box-sizing:border-box;transform:${fundo3D};transform-style:preserve-3d;transition:transform .5s ease" onclick="document.getElementById('${idSvg}').setAttribute('viewBox','0 0 100 100');const tx=document.getElementById('${idSvg}').querySelectorAll('.fcas-pin-txt');tx.forEach(t=>{const g=t.parentElement;const o=g.querySelector('circle').getAttribute('cx');const a=g.querySelector('circle').getAttribute('cy');t.setAttribute('x',o);t.setAttribute('y',a);t.style.fontSize='3px';t.setAttribute('dy','-4');t.style.display='none';t.classList.remove('fcas-fixo')});"><svg id="${idSvg}" viewBox="0 0 100 100" style="width:100%;height:auto;display:block;transition:viewBox .4s ease-in-out" preserveAspectRatio="xMidYMid meet">${sombr3D}<path d="${dPath}" fill="${mapB}" stroke="${mapS}" stroke-width="0.5"/><g>${c}</g></svg></div></div>`}
...
function fcas_lightbox_contato(dados){if(!dados||!dados.nome)return"";const{nome='',cargo='',empresa='',telefones=[],emails=[],redesSociais={},website='',endereco='',notas='',tema='claro'}=dados,e=tema==="escuro",bg=e?'#1e1e1e':'#fff',txt=e?'#e0e0e0':'#333',tit=e?'#fff':'#111',sub=e?'#aaa':'#666',svgF=e?'#aaa':'#555',lnK=e?'#66b2ff':'#0066cc',brd=e?'#333':'#eee',svN=e?'#aaa':'#777',t='<svg style="width:18px;fill:'+svgF+';vertical-align:middle;margin-right:12px" viewBox="0 0 24 24"><path d="M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2z"/></svg>',a='<svg style="width:18px;fill:'+svgF+';vertical-align:middle;margin-right:12px" viewBox="0 0 24 24"><path d="M20 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z"/></svg>',r='<svg style="width:18px;fill:'+svgF+';vertical-align:middle;margin-right:12px" viewBox="0 0 24 24"><path d="M11.99 2C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zm6.93 6h-2.95c-.32-1.25-.78-2.45-1.38-3.56 1.84.42 3.47 1.63 4.33 3.56zM12 4.04c.83 1.2 1.48 2.53 1.91 3.96h-3.82c.43-1.43 1.08-2.76 1.91-3.96zM4.26 14C4.1 13.36 4 12.69 4 12s.1-1.36.26-2h3.38c-.08.66-.14 1.34-.14 2 0 .66.06 1.34.14 2H4.26zm.82 2h2.95c.32 1.25.78 2.45(1.38 3.56-1.84-.42-3.47-1.63-4.33-3.56zm2.95-8H5.08c.86-1.93 2.5-3.14 4.33-3.56-.6 1.11-1.06 2.31-1.38 3.56zM12 19.96c-.83-1.2-1.48-2.53-1.91-3.96h3.82c-.43 1.43-1.08 2.76-1.91 3.96zm2.26-5.96H9.74c-.08-.66-.14-1.34-.14-2 0-.66.06-1.34.14-2h4.52c.07.66.14 1.34.14 2 0 .66-.06 1.34-.14 2zm1.66 5.56c.6-1.11 1.06-2.31 1.38-3.56h2.95c-.86 1.93-2.5 3.14-4.33 3.56zm1.7-5.56c.08-.66.14-1.34.14-2 0-.66-.06-1.34-.14-2h3.38c.16.64.26 1.31.26 2s-.1 1.36-.26 2h-3.38z"/></svg>',o='<svg style="width:18px;fill:'+svgF+';vertical-align:middle;margin-right:12px" viewBox="0 0 24 24"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>',n='<svg style="width:18px;fill:'+svN+';vertical-align:middle;margin-right:12px" viewBox="0 0 24 24"><path d="M14 2H6c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6zm2 16H8v-2h8v2zm0-4H8v-2h8v2zm-3-5V3.5L18.5 9H13z"/></svg>';return`<div class="fcas-card-contato" style="background:${bg};border-radius:12px;padding:30px;box-shadow:0 10px 25px ${e?'rgba(0,0,0,0.5)':'rgba(0,0,0,0.1)'};color:${txt};font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;text-align:center;box-sizing:border-box;display:block;margin:0"><h3 style="margin:0 0 8px 0;font-size:24px;color:${tit}">${nome}</h3>${cargo?`<p class="cargo" style="color:${sub};margin:0 0 24px 0;font-size:16px">${cargo} em <strong>${empresa}</strong></p>`:''}<div class="fcas-lista-dados" style="margin-bottom:25px;display:block">${telefones.map(e=>`<p style="margin:16px 0;padding:4px 0;font-size:15px;line-height:1.4;display:block">${t}<span style="vertical-align:middle">${e}</span></p>`).join('')}${emails.map(e=>`<p style="margin:16px 0;padding:4px 0;font-size:15px;line-height:1.4;display:block">${a}<span style="vertical-align:middle">${e}</span></p>`).join('')}${website?`<p style="margin:16px 0;padding:4px 0;font-size:15px;line-height:1.4;display:block">${r}<a href="${website}" target="_blank" style="color:${lnK};text-decoration:none;vertical-align:middle">${website}</a></p>`:''}${endereco?`<p style="margin:16px 0;padding:4px 0;font-size:15px;line-height:1.4;display:block">${o}<span style="vertical-align:middle">${endereco}</span></p>`:''}</div><div class="fcas-redes" style="margin-top:20px;display:block">${Object.entries(redesSociais).map(([e,s])=>`<a href="${s}" class="fcas-btn-rede" target="_blank" style="display:inline-block;padding:10px 20px;background:#007bff;color:#fff;text-decoration:none;border-radius:6px;font-size:14px;font-weight:bold;margin:5px">${e.toUpperCase()}</a>`).join('')}</div>${notas?`<div class="fcas-notas" style="margin-top:24px;padding-top:16px;border-top:1px solid ${brd};color:${svN};font-size:14px;line-height:1.4;display:block">${n}<span style="vertical-align:middle"><em>${notas}</em></span></div>`:''}</div>`}
function fcas_lightbox_produto(produto){if(!produto||!produto.nome)return "";const{id='',nome='',codigoSku='',precoOriginal=0,precoPromocional=null,descricao='',especificacoes={},imagens=[],valor='',medida='',moeda='R$',tema='claro'}=produto,e=tema==="escuro",bg=e?'#1e1e1e':'#fff',txt=e?'#e0e0e0':'#333',tit=e?'#fff':'#111',skuC='#999',prcA=e?'#2ecc71':'#28a745',dscC=e?'#bbb':'#555',brd=e?'#333':'#f5f5f5',t=precoPromocional?`<span class="fcas-preco-antigo" style="text-decoration:line-through;color:#999;font-size:16px;margin-right:10px">${moeda} ${Number(precoOriginal).toFixed(2)}</span><span class="fcas-preco-atual" style="font-size:24px;color:${prcA};font-weight:bold">${moeda} ${Number(precoPromocional).toFixed(2)}</span>`:`<span class="fcas-preco-atual" style="font-size:24px;color:${prcA};font-weight:bold">${moeda} ${Number(precoOriginal).toFixed(2)}</span>`;return`<div class="fcas-prod-card" style="background:${bg};border-radius:12px;padding:30px;box-shadow:0 10px 25px ${e?'rgba(0,0,0,0.5)':'rgba(0,0,0,0.1)'};color:${txt};font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;text-align:center;box-sizing:border-box;display:block;margin:0"><div class="fcas-prod-galeria" style="margin-bottom:20px;display:block">${imagens.map(e=>`<img src="${e}" alt="${nome}" style="max-width:100%;border-radius:8px;object-fit:cover;margin:0 auto;display:block">`).join('')}</div><div class="fcas-prod-info" style="text-align:left;padding:10px 0;display:block"><h2 style="margin:0 0 8px 0;font-size:26px;color:${tit};display:block">${nome}</h2><div class="fcas-prod-sku" style="color:${skuC};font-size:12px;margin-bottom:20px;display:block">SKU: ${codigoSku}</div>${valor||medida?`<div class="fcas-prod-sku" style="color:${skuC};font-size:12px;margin-bottom:20px;display:block">${valor} ${medida}</div>`:''}<div class="fcas-prod-preco" style="margin:20px 0;display:block">${t}</div><div class="fcas-prod-desc" style="line-height:1.6;margin-bottom:25px;color:${dscC};font-size:15px;word-break:break-word;display:block">${descricao}</div><div class="fcas-prod-specs" style="display:block"><h4 style="margin:0 0 12px 0;font-size:16px;display:block">Specifications</h4><ul style="list-style:none;padding:0;margin:0;display:block">${Object.entries(especificacoes).map(([e,s])=>`<li style="padding:10px 0;border-bottom:1px solid ${brd};font-size:14px;display:block;word-break:break-word"><strong style="float:left">${e}</strong><span style="float:right">${s}</span><div style="clear:both"></div></li>`).join('')}</ul></div></div></div>`}
...
function fcas_lightboc_Pdf(idioma, linkPdf, pg, idElemento) {
    ...
    btnAnterior.addEventListener('click', function() {        if (paginaAtual > 1) carregarPagina(paginaAtual - 1);    });
    btnProximo.addEventListener('click', function() {        if (paginaAtual < totalPaginas) carregarPagina(paginaAtual + 1);    });
...
function fsmodal_clall_menu_arrays(id, dcurrent_id) {
  var fsmodal_clall_menussd = "";    var btunfprecurrent_id = "";   var btunfprnextent_id = "";
...
function fsmodal_call_menu(id) {
    ...
    if (id.target) {
        if (id.target.parentNode) {
            if (id.target.parentNode.getElementsByTagName("button").length > 0) {   fsmodal_call_menu_sd = true;  }
        }
    }
...
var fsmodal_createWi= " margin:0 auto;  width:95%; ";     fsmodal_clall_menussd = "";     dcurrent_id = 0;     is_player0j = "no";     start_fs_mpl = {};       start_fs_postermpl = "";
    ...
    if (optiy) {
        ...
        if (optiy.title && optiy.title != "") {            fsmodal_createdd += `<h1 style="text-align:center;pointer-events:none;color:#fff;font-weight:bold;">${optiy.title}</h1>`;        }
        if (optiy.context) {
            ...
            if (app && app == "yes" && optiy.type && optiy.type == "id") {
                ...
                if (optiy.config) {
                    if (optiy.config.pos && optiy.config.pos != "") {   stdart_fs_mpl_argsd += "&pos=" + optiy.config.pos;  }
            ...
            } else if (optiy.type && optiy.type == "form") {
                is_player0j = "no";
                fsmodal_create_ifir = `<div style="${fsmodal_createWi} height:${Number(optiy.size)}px !important;">  ${fcafs_lightbox_gerarFormularioResponsivo({ method: fcas_lightbox_checkValueEX(optiy,"config","method","GET") || 'GET', action:fcas_lightbox_checkValueEX(optiy,"config","action","") || '', submitText:fcas_lightbox_checkValueEX(optiy,"config","submitText","") || '', onsubmit: fcas_lightbox_checkValueEX(optiy,"config","onsubmit","") || '', lang: fcas_lightbox_checkValueEX(optiy,"config","Lang","en") ||'en', campos: optiy.context })}  </div>`;
            } else if (optiy.type && optiy.type == "contact") {
                is_player0j = "no";
                ...
                is_player0j = "no";
                fsmodal_create_ifir = `<div style="${fsmodal_createWi} height:${Number(optiy.size)}px !important;">  ${fcas_lightbox_mapa({ tema: "claro", tamanhoMapa:"70%", locais: optiy.context || [] })}  </div>`;
...
function registerPlayerOSDEvents(plobf, eventsList, lang = "pt", openf_osdcd_time,osdpositdo,osdposito) {
    ...
    if (!Array.isArray(eventsList) || eventsList.length === 0) {
        for (let eventName in allEvents) {   plobf.OnEvents(eventName, allEvents[eventName]);   } 
    } else {
        for (let i = 0; i < eventsList.length; i++) {
            let eventName = eventsList[i];       if (allEvents[eventName]) {   plobf.OnEvents(eventName, allEvents[eventName]);   }
        }
    }
}
...
function fsmodal_open(optiy){      const num_fsmodal_open=Math.floor(1000+Math.random()*90000);
