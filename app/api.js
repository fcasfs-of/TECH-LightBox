...
function fcas_lightbox_contato(dados){if(!dados||!dados.nome)return"";const{nome='',cargo='',empresa='',telefones=[],emails=[],redesSociais={},website='',endereco='',notas='',tema='claro'}=dados,e=tema==="escuro",bg=e?'#1e1e1e':'#fff',txt=e?'#e0e0e0':'#333',tit=e?'#fff':'#111',sub=e?'#aaa':'#666',svgF=e?'#aaa':'#555',lnK=e?'#66b2ff':'#0066cc',brd=e?'#333':'#eee',svN=e?'#aaa':'#777',t='<svg style="width:18px;fill:'+svgF+';vertical-align:middle;margin-right:12px" viewBox="0 0 24 24"><path d="M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2z"/></svg>',a='<svg style="width:18px;fill:'+svgF+';vertical-align:middle;margin-right:12px" viewBox="0 0 24 24"><path d="M20 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z"/></svg>',r='<svg style="width:18px;fill:none;stroke:'+svgF+';stroke-width:2;stroke-linecap:round;stroke-linejoin:round;vertical-align:middle;margin-right:12px" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><path d="M12 2a14.5 14.5 0 0 0 0 20 14.5 14.5 0 0 0 0-20M2 12h20"/></svg>',o='<svg style="width:18px;fill:'+svgF+';vertical-align:middle;margin-right:12px" viewBox="0 0 24 24"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>',n='<svg style="width:18px;fill:'+svN+';vertical-align:middle;margin-right:12px" viewBox="0 0 24 24"><path d="M14 2H6c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6zm2 16H8v-2h8v2zm0-4H8v-2h8v2zm-3-5V3.5L18.5 9H13z"/></svg>';return`<div class="fcas-card-contato" style="background:${bg};border-radius:12px;padding:30px;box-shadow:0 10px 25px ${e?'rgba(0,0,0,0.5)':'rgba(0,0,0,0.1)'};color:${txt};font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;text-align:center;box-sizing:border-box;display:block;margin:0"><h3 style="margin:0 0 8px 0;font-size:24px;color:${tit}">${nome}</h3>${cargo?`<p class="cargo" style="color:${sub};margin:0 0 24px 0;font-size:16px">${cargo} em <strong>${empresa}</strong></p>`:''}<div class="fcas-lista-dados" style="margin-bottom:25px;display:block">${telefones.map(tel=>`<p style="margin:16px 0;padding:4px 0;font-size:15px;line-height:1.4;display:block">${t}<a href="tel:${tel.replace(/\s+/g,'')}" style="color:${txt};text-decoration:none;vertical-align:middle">${tel}</a></p>`).join('')}${emails.map(email=>`<p style="margin:16px 0;padding:4px 0;font-size:15px;line-height:1.4;display:block">${a}<a href="mailto:${email}" style="color:${lnK};text-decoration:none;vertical-align:middle">${email}</a></p>`).join('')}${website?`<p style="margin:16px 0;padding:4px 0;font-size:15px;line-height:1.4;display:block">${r}<a href="${website}" target="_blank" style="color:${lnK};text-decoration:none;vertical-align:middle">${website}</a></p>`:''}${endereco?`<p style="margin:16px 0;padding:4px 0;font-size:15px;line-height:1.4;display:block">${o}<span style="vertical-align:middle">${endereco}</span></p>`:''}</div><div class="fcas-redes" style="margin-top:20px;display:block">${Object.entries(redesSociais).map(([rede,url])=>`<a href="${url}" class="fcas-btn-rede" target="_blank" style="display:inline-block;padding:10px 20px;background:#007bff;color:#fff;text-decoration:none;border-radius:6px;font-size:14px;font-weight:bold;margin:5px">${rede.toUpperCase()}</a>`).join('')}</div>${notas?`<div class="fcas-notas" style="margin-top:24px;padding-top:16px;border-top:1px solid ${brd};color:${svN};font-size:14px;line-height:1.4;display:block">${n}<span style="vertical-align:middle"><em>${notas}</em></span></div>`:''}</div>`}
function fcas_lightbox_produto(produto){if(!produto||!produto.nome)return "";const{id='',nome='',codigoSku='',precoOriginal=0,precoPromocional=null,descricao='',especificacoes={},imagens=[],valor='',medida='',moeda='R$',tema='claro'}=produto,e=tema==="escuro",bg=e?'#1e1e1e':'#fff',txt=e?'#e0e0e0':'#333',tit=e?'#fff':'#111',skuC='#999',prcA=e?'#2ecc71':'#28a745',dscC=e?'#bbb':'#555',brd=e?'#333':'#f5f5f5',t=precoPromocional?`<span class="fcas-preco-antigo" style="text-decoration:line-through;color:#999;font-size:16px;margin-right:10px;display:inline-block">${moeda} ${Number(precoOriginal).toFixed(2)}</span><span class="fcas-preco-atual" style="font-size:24px;color:${prcA};font-weight:bold;display:inline-block">${moeda} ${Number(precoPromocional).toFixed(2)}</span>`:`<span class="fcas-preco-atual" style="font-size:24px;color:${prcA};font-weight:bold;display:inline-block">${moeda} ${Number(precoOriginal).toFixed(2)}</span>`;return`<div class="fcas-prod-card" style="background:${bg};border-radius:12px;padding:30px;box-shadow:0 10px 25px ${e?'rgba(0,0,0,0.5)':'rgba(0,0,0,0.1)'};color:${txt};font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;text-align:center;box-sizing:border-box;display:block;margin:0"><div class="fcas-prod-galeria" style="margin-bottom:20px;display:block">${imagens.map(e=>`<img src="${e}" alt="${nome}" style="max-width:100%;border-radius:8px;object-fit:cover;margin:0 auto;display:block">`).join('')}</div><div class="fcas-prod-info" style="text-align:center;padding:10px 0;display:block"><h2 style="margin:0 0 8px 0;font-size:26px;color:${tit};display:block">${nome}</h2>${codigoSku?`<div class="fcas-prod-sku" style="color:${skuC};font-size:12px;margin-bottom:20px;display:block">SKU: ${codigoSku}</div>`:''}${valor||medida?`<div class="fcas-prod-sku" style="color:${skuC};font-size:12px;margin-bottom:20px;display:block">${valor} ${medida}</div>`:''}<div class="fcas-prod-preco" style="margin:20px 0;display:block">${t}</div><div class="fcas-prod-desc" style="line-height:1.6;margin-bottom:25px;color:${dscC};font-size:15px;word-break:break-word;display:block">${descricao}</div>${especificacoes&&Object.keys(especificacoes).length>0?`<div class="fcas-prod-specs" style="display:block"><h4 style="margin:0 0 12px 0;font-size:16px;display:block;text-align:center">Specifications</h4><ul style="list-style:none;padding:0;margin:0;display:block">${Object.entries(especificacoes).map(([e,s])=>`<li style="padding:10px 0;border-bottom:1px solid ${brd};font-size:14px;display:block;word-break:break-word;text-align:center"><strong style="margin-right:5px">${e}:</strong><span>${s}</span></li>`).join('')}</ul></div>`:''}</div></div>`}
