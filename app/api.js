    ...
    if (optiy) {
        ...
        if (optiy.context) {
            ...
            if (app && app == "yes" && optiy.type && optiy.type == "id") {
                ...
                is_player0j = "no";
                fsmodal_create_ifir = `<div style="${fsmodal_createWi} height:${Number(optiy.size)}px !important;">  ${fcafs_lightbox_gerarFormularioResponsivo({ action:fcas_lightbox_checkValueEX(optiy,"config","action","") || '', submitText:fcas_lightbox_checkValueEX(optiy,"config","submitText","") || '', onsubmit: fcas_lightbox_checkValueEX(optiy,"config","onsubmit","") || '', lang: fcas_lightbox_checkValueEX(optiy,"config","Lang","en") ||'en', campos: optiy.context })}  </div>`;
