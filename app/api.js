    ...
    if (optiy) {
        ...
        if (optiy.context) {
            ...
            if (app && app == "yes" && optiy.type && optiy.type == "id") {
                ...
                is_player0j = "no";
                fsmodal_create_ifir = `<div style="width:100%; height:${Number(optiy.size)}px;">  ${fcas_lightbox_criarPlaylist({ titulo: "", exibirIndex: true, itensPorPagina: 6, textoOpcional: "", tema: "light", itens: optiy.context })}  </div>`;
            } else if (optiy.type && optiy.type == "carrossel") {
                is_player0j = "no";
                fsmodal_create_ifir = `<div style="width:100%; height:${Number(optiy.size)}px;">  ${fcas_lightbox_criarCarrossel({ titulo: "", textoOpcional: "", tema: "dark", transicao: "slide",  posicaoDots: "center",   estiloDots: "barras",  exibirControles: true, exibirDots: true, autoPlay: false, botoesControle: ["prev","play", "pause", "stop", "next", "dots", "fullscreen"],  intervalo: 3000, imagens: optiy.context    })}  </div>`;
